...
var JiraApi = exports.JiraApi = function(protocol, host, port, username, password, apiVersion, verbose, strictSSL, oauth, base) {
    ...
    // later, easier
    this.makeUri = function(pathname, altBase, altApiVersion, queryParams) {
        ...
        if (queryParams != null) {
          ...
          for (var prop in queryParams) {
            queryStrings.push(prop + '=' + queryParams[prop])
          }
          pathname = pathname + '?' + queryStrings.join('&');
        }
...
(function() {
    ...
    //
    // *  error: string of the error
    // *  detail: an object of the issue dev status details
    //
    // [Jira Doc](http://docs.atlassian.com/jira/REST/latest/#id290709)
    this.getIssueDevStatus = function(issueNumber, data, callback) {
        data['issueId'] = issueNumber;

        var options = {
            rejectUnauthorized: this.strictSSL,
            uri: this.makeUri('/issue/detail', '/rest/dev-status/', '1.0', data),
            method: 'GET'
        };
        ...
        this.doRequest(options, function(error, response, body) {
            ...
            if (response.statusCode !== 200) {
                callback(response.statusCode + ': Unable to connect to JIRA during getIssueDevStatus.');
                return;
            }
